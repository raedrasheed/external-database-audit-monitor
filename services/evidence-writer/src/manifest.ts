...
export function buildSegmentManifest(seg: SealReadySegment, opts: BuildSegmentManifestOptions): SegmentManifest {
  ...
  //
  // M1 (T111 review): every embedded sub-structure is DEEP-COPIED so the manifest
  // shares no mutable reference with the input SealReadySegment / previousSegment.
  // Mutating the inputs after build cannot change the manifest content, its
  // manifest_hash, or its validation result.
  const core = {
    ...
    object_list: structuredClone(seg.object_list) as SegmentObjectRef[],
    object_hash_list: structuredClone(seg.object_hash_list) as SegmentObjectHash[],
  ...
  // Deep-freeze so the returned manifest is immutable (defense-in-depth: a hashed
  // evidence structure must not be mutable after its manifest_hash is fixed).
