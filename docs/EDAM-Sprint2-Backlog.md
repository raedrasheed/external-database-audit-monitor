...
### 13.2y E2E-WV-CONFORMANCE — T148 WV-1…WV-14 conformance findings (from the accepted adversarial review)

T148 review verdict: APPROVE WITH CHANGES — the WV-1…WV-14 conformance suite (`conformance/wv/`) drives the spec §17.2 cases against the REAL frozen implementations (writer/seal model, `DevEd25519Signer`, `DevRfc3161Provider` + `requestAnchor`, T133 `buildAnchorRecord`, the §10 verifier, the T145 export builder, the T146 verifier CLI), with the only approved fakes being `InMemoryWormStore` (WV-8) and `FakeAnchorProvider` outage (WV-13). The suite is honesty-first and non-vacuous: WV-12/WV-14 assert a fail-closed `overall_result === 'PASS'` on a real anchored export, and every negative asserts a located FAIL with non-empty `offending_ids`. **No Critical findings. No High findings. No Medium findings. T148 accepted with changes.** **WV-1…WV-14 are green** (14/14; gate exit 0; full suite 677 passed / 9 skipped; determinism rig exit 0; coverage 93.15 / 85.69 / 95.19 / 93.15). **The `evidence-conformance` gate is accepted** (fails non-zero on any failed/missing WV case; non-vacuous via the required-id guard; dual CI job + in-suite enforcement; report artifact gitignored). **Located failures on tamper are proven.** **The E2E epic (T140–T148) is complete and green.** **Sprint 2 is NOT yet complete** — Epic E2F (T160–T164: live compose, live evidence harness, live drills, live-stack validation, security sign-off) remains outstanding, and per §13.3 the live WV-8/WV-13 enforcement + the MinIO WORM hardening (H1/H2/H3/H5) are prerequisites for sign-off. **No mandatory fixes are required before T160.** No verifier/export/schema/signing/anchoring behavior was modified (footprint is `conformance/**` + CI/script wiring).

| ID | Severity | Finding | Disposition |
|---|---|---|---|
| **T148-L1** | Low | **WV-2 conformance diverges from spec §17.2 literal wording.** A pure content tamper localizes to `per_object_hash` FAIL only; `object_chain` stays PASS (confirmed in `cce-model/verify.ts`: `row_hash_ok = rowHash(prev, cce.evidence.event_hash)` uses the STORED event_hash). The spec says both fail. Security-equivalent — `per_object_hash` (content) and `object_chain` (stored-triple self-consistency + linkage) are orthogonal and jointly exhaustive; no tamper escapes. | **Track** to either reconcile the spec wording or harden `object_chain` to recompute `row_hash` against the freshly recomputed `event_hash`. Non-blocking. |
| **T148-L2** | Low | **WV-3 missing-object location is segment-level** (`segment_manifest` FAIL); the withheld `worm_object_key` appears only in the harness detail string, not in `offending_ids` (spec §17.2 says "FAIL with the offending key"). | Q7-approved; **track** for a future verifier key-level location hardening. Non-blocking. |
| **T148-L3** | Low | **Negative cases assert only the targeted check FAILs**, not that the OTHER checks remain PASS (surgical-isolation not fully proven; WV-2/WV-7/WV-11 partially do). | Strengthen negatives (e.g., WV-4 assert `per_object_hash` PASS; WV-6 assert `hsm_signature` PASS) so each mutation is proven surgical. Non-blocking. |
| **T148-L4** | Low | **Minor robustness:** WV-5(b) uses an inline `{ get: () => undefined }` mock trust directory rather than `InMemorySigningKeyDirectory([])`; WV-6 casts `anchor_provider` to rfc3161 (brittle if the fixture provider changes); `expectFailLocated` uses substring `.includes()` matching; the gate's `void main()` lacks a `.catch` (consistent with sibling gates; no realistic reject path). | Cosmetic hardening during a later pass. Non-blocking. |

**Notes (non-findings, approved/known):** WV-8 is in-memory only (live MinIO enforcement → T162); WV-10 uses option-A non-overlap no-missing-event semantics (Q5; matches T142's deferred engine-specific arithmetic); WV-12/WV-14 depend on the approved object-sidecar model (T146 Q1); WV-12 inherits the T147-L1…L5 isolation-proof gaps; CT1 transparency-log equivocation remains out of scope.

### 13.2z E2F-LIVE-STACK — T160 dev compose findings (from the accepted adversarial review)

T160 review verdict: APPROVE WITH CHANGES — the M-Live dev-compose extension is **infrastructure-only**: `minio-init` (one-shot Object-Lock bucket bootstrap) + the `dev-signer` / `dev-tsa-log` **dev topology/health stand-ins** (zero-dependency Node `.mjs` stubs, public images, no Dockerfile) + a structural `compose-validate` gate. **No Critical findings. No High findings. No Medium findings. T160 accepted with changes.** **T160 is infrastructure-only** (footprint: `deploy/compose/**` + `conformance/scripts/compose-validate.ts` + CI/script/docs/eslint wiring; no product TS, no schemas). **No verifier, export, signing, anchoring, schema, or product behavior changed.** **The dev compose topology is correct at config level** (`docker compose config` renders 10 services; the assertion confirms required services + healthchecks + bootstrap + stand-ins). **`minio-init` provisions an Object-Lock bucket correctly on the fresh-bucket path** (`mc mb --with-lock` + versioning + default COMPLIANCE retention). **The `dev-signer` / `dev-tsa-log` services are stand-ins only and cannot become the real crypto path without future code changes** (no product wiring; even on misuse they sign raw bytes with an ephemeral untrusted key ≠ the canonical anchor message ⇒ the verifier fail-closes, never a forged PASS). **CI wiring is correct.** **Healthchecks are correct at default ports.** **T161 was not started; T161 is unblocked; no mandatory fixes are required before T161.** **Full suite (677 passed / 9 skipped), determinism rig (exit 0), and all gates (incl. the new `compose-validate`) were green.**

| ID | Severity | Finding | Disposition |
|---|---|---|---|
| **T160-L1** | Low | **Port-override foot-gun for `dev-signer` / `dev-tsa-log`:** the healthcheck and the container-side port mapping are hardcoded to 8090/8091 while the server reads `DEV_SIGNER_PORT` / `DEV_TSA_LOG_PORT`. Defaults work, but overriding the env can make the service unhealthy or mis-mapped. | Derive the container port + healthcheck from the env var, or document fixed internal ports. Non-blocking. |
| **T160-L2** | Low | **`minio-init` does not enable or verify Object Lock on a pre-existing bucket.** `mc mb --with-lock` only applies at bucket creation; if the bucket already exists without Object Lock, the script continues without failing. | Add a post-bootstrap retention/Object-Lock assertion that fails loudly if lock is absent. Non-blocking (fresh dev volumes; live enforcement validated in T161/T162). |
| **T160-L3** | Low | **`compose-validate` is structural-only and not in-suite.** It asserts required services, healthchecks, and bootstrap declarations exist, but does not prove runtime health; enforced through CI only (it needs Docker). | Add a docker-free unit test of `checkComposeConfig` with planted missing-service controls and guard `main()`. Non-blocking. |
| **T160-L4** | Low | **Unpinned `:latest` image tags** for `minio/mc:latest`, `node:20-alpine`, and the pre-existing `minio/minio:latest`. | Pin images for reproducibility. Non-blocking; consistent with the existing T104 convention. |

**Notes (non-findings):**
- **T160-L5** — The `dev-signer` is a generic "sign arbitrary bytes" oracle and the stubs bind all interfaces / host ports. **Not a security risk:** the key is ephemeral and untrusted, the signed bytes are not the canonical anchor message, and the services are documented dev-only stand-ins.
- **T160-L6** — `eslint.config.js` ignores `deploy/**`. Acceptable today because `deploy/` is infra-only, but future real code under `deploy/` would not be linted.

### 13.2aa E2F-LIVE-EVIDENCE — T161 live evidence harness findings (from the accepted adversarial review)

T161 review verdict: APPROVE WITH CHANGES — the live evidence path was **independently verified** against the live MinIO store (re-probed over the persisted `minio-data` volume): 3 real CCEs written across 2 segments (genesis + successor), read back live, with `evidence.worm_object_key` + `evidence.segment_id` populated in the live WORM object, versioning enabled, and a **non-fabricated** anchor token (the committed `tsa_token` decodes to a TstInfo whose `message_imprint` equals `anchor_ref.head_hash` — it commits to the anchored head). The temporary harness was **not committed** (commit footprint = `docs/evidence/` only). **No Critical findings. No High findings.** **T161 accepted with changes.** **T162 remains unblocked; no mandatory fixes are required before T162** (the record corrections are recommended before T163 cites the artifact). **Live evidence path independently verified** (3 live objects / 2 segments / live read-back / no fabrication). **`anchor_ref` is not persisted in the immutable WORM object and exists only in the post-anchor view** (architecturally forced — WORM is append-only; `anchor_ref` is computed after sealing). **Overwrite rejection came from the adapter's existence guard (`if exists throw`), not object-level S3 retention enforcement** (objects carry no ObjectLock retention — `headObjectLock` returns no configuration; consistent with the §13.3 H1/H2/H3/H5 gating). **Snapshot-phase CCEs were not exercised** (all 3 CCEs streaming-phase; disclosed caveat per Q2). **Live-vs-in-process reporting remains accurate** (Live = MinIO WORM write/read; In-process = dev signer + dev anchor provider, explicitly not called external live services).

| ID | Severity | Finding | Disposition |
|---|---|---|---|
| **T161-M1** | Medium | **`anchor_ref` is not in the live WORM object.** The live-stored CCE carries only `worm_object_key` + `segment_id`; `anchor_ref` was built after the WORM write and set on the in-memory post-anchor view only (never persisted — WORM is immutable). The committed record's `read_back_from_live_minio.all_back_refs_populated: true` and the md "evidence.anchor_ref populated live: yes" overstate this (the harness's check covered only `worm_object_key` + `segment_id`). Architecturally correct (anchor_ref belongs in the projection/anchor-record), but the record is misleading. | Correct the artifact wording (distinguish "in live WORM object: worm_object_key, segment_id" vs "on the post-anchor projection view, derived from live anchoring: anchor_ref") before T163 cites it. Non-blocking for T162. |
| **T161-M2** | Medium | **Objects are not S3-Object-Lock-retained.** `headObjectLock` on the live object returns "does not have a ObjectLock configuration" — the harness passed `{retentionMode:'compliance'}` without `retainUntil`, so `putObjectRetention` never ran. `object_lock: true` reflects the bucket capability only, and `live_worm_immutability_overwrite_rejected: true` is the adapter's app-level existence guard, not S3 Object Lock. The record overstates store-enforced immutability. | Correct the wording; aligns with the §13.3 gating (store-level WORM enforcement deferred to H1/H2/H3/H5 + T164). Non-blocking for T162. |
| **T161-L1** | Low | **Acceptance flags + WV-1 / WV-11 + no-fabrication are self-attested** by the non-committed harness and not independently re-derivable from the committed artifact (it omits manifests, object hashes, and the published dev TSA cert). `acceptance.objects_anchored_end_to_end` is derived from `segments >= 2` — a weak proxy, not a real per-segment anchor check. | Have the record carry `manifest_hash`/genesis fields + the dev TSA cert so the checks are offline-reproducible. Non-blocking. |
| **T161-L2** | Low | **"streaming + cce-1.1 snapshot" not fully satisfied** — 0 snapshot-phase CCEs (all streaming). Honestly disclosed as a caveat (Q2 scoped snapshot assembly out), but the description's wording is not met. | Exercise a snapshot-phase CCE (snapshot-epoch assembly) in a later live drill if required. Non-blocking. |

### 13.2ab E2F-OFFLINE-DRILLS — T162 offline verifier + tamper drills findings (from the accepted adversarial review)

T162 review verdict: APPROVE — the strongest E2F deliverable to date. The valid offline PASS is **independently reproducible from committed artifacts** (the reviewer re-ran the actual `verifier-cli` binary on the committed `base-export-package.json` + `objects/` + `trust.json` → exit 0, all eight §10 checks PASS, envelope PASS, matching `valid-report.json`), and **all nine tamper drills were independently verified** (D1/D2/D3/D6 re-derived exactly from the committed base; D4/D5 detection re-derived + genuine isolated-form CLI reports; D7 genuine report corroborated by T148 WV-10; D8 re-run denied by live MinIO/WORM; D9 re-run pending/no-token/builder-refused in-process) with **no overstatement** — including the honestly-disclosed, reproducible segment-gap-via-`export_signature` mechanism (the CLI derives scope from the manifests present, so `no_missing_segment` stays PASS while the dropped manifest is caught by the package_hash recompute). **No Critical findings. No High findings. No Medium findings. T162 accepted.** **Offline verifier PASS is independently reproducible.** **All drills independently verified.** **T162 satisfies the acceptance criteria** (valid chain PASSes offline; every drill fails-as-expected). **No verifier / export / signing / anchoring / schema behavior changed** (footprint: `docs/evidence/t162/**` + the minimal reproducibility test only). **No private material committed** (trust carries only public keys/cert; secret-hygiene green). **Temporary harness absent and not committed.** **Live = MinIO WORM write/read/legal-hold-deny; Offline = verifier-cli binary; In-process (NOT external live services) = dev signer + dev anchor providers.** **T163 is unblocked; no mandatory fixes are required before T163.** **Full suite (678 passed / 9 skipped), determinism rig (exit 0), and all gates (incl. secret-hygiene) green.**

| ID | Severity | Finding | Disposition |
|---|---|---|---|
| **T162-L1** | Low | **Drills 4/5/7 used fresh per-drill packages** (distinct segment ids `…d4/d5/d7`) signed with an **ephemeral, uncommitted export key**; their isolated-form outcomes (`export_signature` PASS) are **not reproducible from committed artifacts alone**. A third party can reproduce the **detection** (post-build corruption ⇒ §10 check FAIL + envelope FAIL) but not the isolated form. The committed per-drill reports are genuine CLI outputs but stand alone (not tied to the committed base). | Note in the artifact which drills are reproducible-from-committed vs report-only. Non-blocking. |
| **T162-L2** | Low | **The committed reproducibility test re-verifies via `runExportVerification`** (the CLI's in-process orchestration core), **not the CLI binary subprocess.** It proves the committed package passes the verification logic + exit-0 mapping, but does not exercise the binary's process/exit path in CI (the binary path was independently verified during the review). Minor vs D2's "actual binary." | Optionally invoke the binary in the test. Non-blocking. |
| **T162-L3** | Low | **Self-attested operational counters:** the summary's `live.writes=3` / `reads_back_ok=3` and the D8/D9 outcomes are **not reproducible from committed inputs** (they are live / in-process). Corroborated (the committed sidecar objects are the written CCEs and verify; the reviewer re-ran D8/D9). Inherent to live validation; disclosed. | None required; inherent to the live task. Non-blocking. |

**Notes (non-findings):** the streaming-only caveat (D5 decision) and the segment-gap-via-`export_signature` mechanism (D6) are honestly disclosed and matched the reviewer's independent re-derivation.
